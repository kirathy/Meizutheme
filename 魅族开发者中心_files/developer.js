...
(function ($) {
    ...
    function makeLayOuter(jop) {
        ...
        var angle = opt.host.siblings('.m_angle_down');
        if (angle && angle.length) {
            angle.click(function () {
                opt.host.trigger('click');
            });
        }
        return layOuter;
    }
    ...
    makeLayOuter._fns = {
        ...
        '_show': function (e) {
            ...
            opt.host.siblings('.m_angle_down').length && makeLayOuter._hover(host.data("lj"), opt.host.siblings('.m_angle_down'), opt.timeout, opt.end, extra, opt.extraId);
    ...
    Select.prototype = {
        ...
        _eventBind: function (opt, host, w) {
            ...
            $('li', host.data('lj')).each(function () {
                ...
                if(title =='查看主题任务'){ $(this).find("div").addClass("themeTask");}
                ...
                if (event.type == 'click') {
                    ...
                    if(title !='查看主题任务'){
                        ...
                        opt.click.call(proto.options.curObj, {"value": "javascript:void(0);", "text": text, 'jobj': host});
                        $(this).parent().parent().hide();
                        $('.mzContainer').hide();
                        $(this).find("div").addClass("themeTask");
                    }
    ...
    Dialog.prototype = {
        open: function () {
            ...
            opt['min-height'] && (style1['min-height'] = opt['min-height']);
    ...
    $.block = {
        ...
        getCenterTL: function (_w, _h) {
            var wh = this.getVisableWH(),
                sTop = Math.max(document.body.scrollTop, document.documentElement.scrollTop) -100;
            return {
    ...
    window['JAlertGetContent'] = function (type) {
        if (!window['JQAlert']) {
            ...
            cache.push('<div id="mzAlert_loading" class="loading"><div class="loading_message"></div><img src="/resources/designer/images/common/loading.gif" class="loading_pic" /></div>');
    ...
    window['jAlert'] = function (msg, title, callback) {
        ...
        if(msg.length > '200'){
            $('.part1 .alert_message').addClass('message_more');
        } else {
            $('.part1 .alert_message').removeClass('message_more');
        }

        window.JQAlert.alertFn = callback;
        $('.m_btn1', jQobj).unbind('click').click(function () {
    ...
    window['jLoading'] = function (msg) {
        ...
        if (!window.JQAlert.loading) {
            window.JQAlert.loading = jQobj.mzDialog({'nohide': true, 'width': 350, 'height': 79, 'min-height': 79});
        }
            ...
            for (var ele; ele = inputAry[--len];) {
                ...
                if (cover) {
                    ...
                    if (switchover) {
                        ...
                        switchoverFn.append($('<a href="javascript:;" style="display:block; width:26px; height:14px; border-bottom:1px solid #b2bdc7; background:url(/resources/designer/images/register/arrow.png) no-repeat 0 0"></a>').click(plugins.fn.input.up));
                        switchoverFn.append($('<a href="javascript:;" style="display:block; width:26px; height:14px; background:url(/resources/designer/images/register/arrow.png) no-repeat 0 -15px"></a>').click(plugins.fn.input.down));
...
function w_app_loaded() {
    ...
    $flyme.mouseover(function () {
        ...
        "data": [
            {
                'value': '/console/account/info?t='+new Date().getTime(),
                'text': '账户管理'
            },
            {
            	'value': ('https://member.meizu.com/logout.jsp?useruri=' + encodeURIComponent(window.location.href)),
                'text': '退出'
            }
        ],
        "pos": 'down',
        ...
        $("#noPrompt").live("click", function () {
            $.get("/console/message/hide", function (data) {
                ...
                if (result && data.code == 200) {
                    ...
                    notifyHtml = $("#notifyTemplate").tmpl(notify);
                    if (dataLen > 0) {
                        $notify.mzSelect({"isModify": true,"dataIsHtml": true, "data": notifyHtml});
                    }
                    else {
...
$(function () {
    ...
    $('.menu_row').click(function () {
        ...
        //特殊处理
        for(var i=1;i<10;i++){
            if($('#menu_li_'+i).hasClass('menu_selected')){
                $('#menu_li_'+i).addClass('menu_selected');
                $('#menu_li_'+i).next().show();
                $('#menu_li_'+i).parent().parent().show();
            }
        }
    });
    ...
    $('#j_subNav .nav-more').hover(function(){
        $(this).addClass('nav-more-on');
    },function(){
        $(this).removeClass('nav-more-on');
    });
